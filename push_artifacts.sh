...
cat broadcast/$1/run-latest.json out/GuardianFactory.sol/GuardianFactory.json | \
jq -s \
    'add | 
    { chain: .chain} * (.transactions[] |
    { transactionType, contractName, contractAddress } |
    select(.transactionType == "CREATE" and .contractName == "GuardianFactory") |
    {contractName, contractAddress}) * {abi: .abi}' > web/src/artifacts/GuardianFactory.json
